...
class LinfPGDAttack:
  ...
  def perturb(self, x_nat, y, sess, trans=None):
    ...
    if trans is None:
        trans = np.zeros([len(x_nat), 8])
        # trans = np.zeros([len(x_nat), 3])

    # SL3 change 8
    # no_op = np.zeros([len(x_nat), 3])
    no_op = np.zeros([len(x_nat), 8])
  ...
  def perturb(self, x_nat, y, sess):
    ...
    if self.loss_function == "reg_kl":
      ...
      for i in range(1):
          ...
          grad_adv = grad[n:2*n]
          # t = np.add(t, [self.step_size] * np.sign(grad_adv), out=t, casting='unsafe')
          t = np.add(t, [self.step_size] * grad_adv, out=t, casting='unsafe')
          t = np.clip(t, -lim_arr, lim_arr)
    else:
      for i in range(1):
        curr_dict = {self.model.x_input: x_nat,
        ...
        t = np.add(t, [self.step_size] * grad, out=t, casting='unsafe')
